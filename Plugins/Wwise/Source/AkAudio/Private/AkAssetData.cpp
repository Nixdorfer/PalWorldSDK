...
void UAkAssetDataWithMedia::FillFinalMediaList()
{
	...
	{
		if (auto* LoadedMedia = tempMedia.LoadSynchronous())
		{
			MediaList.Add(LoadedMedia);
		}
		else
		{
			UE_LOG(LogAkAudio, Error, TEXT("UAkAssetDataWithMedia::FillFinalMediaList: Could not load media asset %s, it was not added to the media list."), *tempMedia.GetAssetName())
		}
	}
}
...
bool UAkAssetDataSwitchContainerData::IsReadyForAsyncPostLoad() const
{
	...
	{
		if (child && !child->IsReadyForAsyncPostLoad())
		{
			return false;
		}
	}
	...
	{
		if (entry && !entry->IsReadyForAsyncPostLoad())
		{
			return false;
		}
	}
...
void UAkAssetDataSwitchContainer::LoadSwitchContainer(UAkAssetDataSwitchContainerData* SwitchContainer)
{
	if (SwitchContainer)
	{
		...
		auto* LoadedGroupValue = SwitchContainer->GroupValue.Get();
		const bool bIsGroupValueValid = IsValid(LoadedGroupValue);
		uint32 SwitchID = AK_INVALID_UNIQUE_ID;
		if(bIsGroupValueValid)
		{
			SwitchID = LoadedGroupValue->ShortID;
		}
		else
		{
			...
			if (FullName.FindLastChar('-', Position))
			{
				...
				SwitchID = FAkAudioDevice::GetIDFromString(ValueName);
			}
		}
		SwitchIDToDataMap.Add(SwitchID, TWeakObjectPtr<UAkAssetDataSwitchContainerData>(SwitchContainer));
		...
		const bool bIsGroupValueUnset = SwitchContainer->GroupValue.IsNull();
		if ((bIsGroupValueValid || bIsGroupValueUnset))
		{
			for (UAkMediaAsset* Media : SwitchContainer->MediaList)
			{
				if (IsValid(Media))
				{
					...
					if (!bIsGroupValueUnset)
					{
						SwitchContainer->GroupValue.Get()->SignalMediaDependencyIsLoaded(Media);
					}
				}
			}

			LoadSwitchContainer(SwitchContainer->Children);
			FAkAudioDevice::GetOnSwitchValueLoaded(SwitchID).RemoveAll(this);
			FAkAudioDevice::GetOnSwitchValueUnloaded(SwitchID).AddUObject(this, &UAkAssetDataSwitchContainer::OnSwitchValueUnloaded);
		}
		else
		{
...
{
	if (NewGroupValue && IsValid(NewGroupValue))
	{
		TArray<TWeakObjectPtr<UAkAssetDataSwitchContainerData>> GroupValueSwitchContainers;
		SwitchIDToDataMap.MultiFind(NewGroupValue->ShortID, GroupValueSwitchContainers);
		for (auto SwitchContainerToLoad : GroupValueSwitchContainers)
		{
			if (SwitchContainerToLoad.IsValid(false, true))
			{
				LoadSwitchContainer(SwitchContainerToLoad.Get());
			}
		}
	}
}

void UAkAssetDataSwitchContainer::OnSwitchValueUnloaded(UAkGroupValue* NewGroupValue)
{
	FAkAudioDevice::GetOnSwitchValueLoaded(NewGroupValue->ShortID).AddUObject(this, &UAkAssetDataSwitchContainer::OnSwitchValueLoaded);
}

void UAkAssetDataSwitchContainer::BeginDestroy()
{
	for (auto Pair : SwitchIDToDataMap)
	{
		uint32 SwitchValueID = Pair.Key;
		FAkAudioDevice::GetOnSwitchValueLoaded(SwitchValueID).RemoveAll(this);
		FAkAudioDevice::GetOnSwitchValueUnloaded(SwitchValueID).RemoveAll(this);
	}
	SwitchIDToDataMap.Reset();
	Super::BeginDestroy();
}
