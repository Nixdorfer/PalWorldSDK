...
TMap<uint32, TSet<TWeakObjectPtr<UAkAudioEvent>>> FAkAudioDevice::PlayingIDToPinnedAudioEventMap;
TMap<uint32, TSet<TWeakObjectPtr<UAkExternalMediaAsset>>> FAkAudioDevice::PlayingIDToPinnedExternalSourceMap;
TMap<uint32, FSwitchValueLifetimeEvent> FAkAudioDevice::OnSwitchValueLoadedMap;
TMap<uint32, FSwitchValueLifetimeEvent> FAkAudioDevice::OnSwitchValueUnloadedMap;
...
void FAkAudioDevice::Teardown()
{
	...
	if (m_bSoundEngineInitialized)
	{
		...
		{
			delete MediaMemoryManager;
			MediaMemoryManager = nullptr;
		}
...
AkPlayingID FAkAudioDevice::PostEvent(
	...
	EAkAudioContext in_AudioContext
)
{
	...
	if (m_bSoundEngineInitialized && CallbackManager)
	{
		...
		if (pPackage)
		{
			...
			else
			{
				...
				PlayingIDToAudioContextMap.Add(playingID, in_AudioContext);
			}
		}
	}
...
)
{
	...
	if (m_bSoundEngineInitialized && in_pGameObject && CallbackManager)
	{
		if (in_pGameObject->VerifyEventName(in_EventName) && in_pGameObject->AllowAudioPlayback())
		{
			...
			return PostEvent(in_EventName, gameObjID, in_ExternalSources, CreateCallbackPackage, in_AudioContext);
		}
	}
...
AkPlayingID FAkAudioDevice::PostEvent(
	...
	const EAkAudioContext in_AudioContext	 /* = GameplayAudio */
    )
{
	if (!in_pEvent)
	...
	AkPlayingID playingID = PostEvent(in_pEvent->GetName(), in_pActor, in_uFlags, in_pfnCallback, in_pCookie, in_bStopWhenOwnerDestroyed, in_ExternalSources, in_AudioContext);
	if (playingID != AK_INVALID_PLAYING_ID)
	{
...
AkPlayingID FAkAudioDevice::PostEvent(
	...
	bool in_bStopWhenOwnerDestroyed, /*= false*/
	const TArray<AkExternalSourceInfo>& in_ExternalSources, /* = TArray<AkExternalSourceInfo>()*/
	const EAkAudioContext in_AudioContext /* Gameplay Audio */
    )
{
	if (m_bSoundEngineInitialized)
	{
		...
		else if (!in_pActor->IsActorBeingDestroyed() && IsValid(in_pActor))
		{
			...
			if (pComponent)
			{
				...
				return PostEvent(in_EventName, pComponent, in_uFlags, in_pfnCallback, in_pCookie, in_ExternalSources, in_AudioContext);
			}
		}
	}
...
)
{
	if (m_bSoundEngineInitialized)
	{
		...
		else if (!in_pActor->IsActorBeingDestroyed() && IsValid(in_pActor))
		{
			...
			{
				pComponent->StopWhenOwnerDestroyed = in_bStopWhenOwnerDestroyed;
				return PostEvent(in_EventName, pComponent, PostEventCallback, in_uFlags, in_ExternalSources, in_AudioContext);
			}
		}
	}
...
AkPlayingID FAkAudioDevice::PostEventLatentAction(
	...
	const TArray<AkExternalSourceInfo>& in_ExternalSources /* = TArray<AkExternalSourceInfo>()*/
	, const EAkAudioContext in_AudioContext
)
{
	if (m_bSoundEngineInitialized)
	{
		...
		else if (!Actor->IsActorBeingDestroyed() && IsValid(Actor))
		{
			...
			if (pComponent)
			{
				...
				return PostEventLatentAction(EventName, pComponent, LatentAction, in_ExternalSources, in_AudioContext);
			}
		}
	}
...
AkPlayingID FAkAudioDevice::PostEvent(
	...
	void * in_pCookie, /*= NULL*/
	const TArray<AkExternalSourceInfo>& in_ExternalSources, /*= TArray<AkExternalSourceInfo>()*/
	EAkAudioContext in_AudioContext /*= GameplayAudio*/
	)
{
...
)
{
	...
	FFunctionGraphTask::CreateAndDispatchWhenReady([this, AudioEvent, Actor, PostEventCallback, CallbackFlags, bStopWhenOwnerDestroyed, ExternalSources, PlayingIDPromiseCopy(MoveTemp(PlayingIDPromise)), AudioContext]() mutable 
		{
			AkPlayingID PlayingID = AK_INVALID_PLAYING_ID;
			...
			if (m_bSoundEngineInitialized && AudioEvent && IsValid(AudioEvent))
			{
				...
				else if (!Actor->IsActorBeingDestroyed() && IsValid(Actor))
				{
					...
					if (AkComponent)
					{
						...
						PlayingID = PostEvent(AudioEvent->GetName(), AkComponent, PostEventCallback, CallbackFlags, ExternalSources->ExternalSourceArray, AudioContext);
...
)
{
	...
	}).Then([this, AudioEvent, GameObject, PostEventCallback, CallbackFlags, ExternalSources, AudioContext](auto PreviousFuture) {
...
)
{
	...
	FFunctionGraphTask::CreateAndDispatchWhenReady([this, in_pEvent, in_Location, in_Orientation, in_World, playingIDPromiseCopy(MoveTemp(playingIDPromise)), in_AudioContext]() mutable {
		AkPlayingID playingID = AK_INVALID_PLAYING_ID;

		if (in_pEvent && IsValid(in_pEvent))
		{
			playingID = PostEventAtLocation(in_pEvent->GetName(), in_Location, in_Orientation, in_World, in_AudioContext);
			if (playingID != AK_INVALID_PLAYING_ID)
			{
				in_pEvent->PinInGarbageCollector(playingID);
			}
...
)
{
	...
	FFunctionGraphTask::CreateAndDispatchWhenReady([this, AudioEvent, Actor, bStopWhenOwnerDestroyed, LatentAction, ExternalSources, PlayingIDPromiseCopy(MoveTemp(PlayingIDPromise)), in_AudioContext]() mutable {
		AkPlayingID PlayingID = AK_INVALID_PLAYING_ID;
...
)
{
	auto PlayingIDFuture = Async(EAsyncExecution::TaskGraph, [AudioEvent] 
		...
		}).Then([this, AudioEvent, AkComponent, LatentAction, ExternalSources, in_AudioContext](auto PreviousFuture)
		{
			if (m_bSoundEngineInitialized && AudioEvent && IsValid(AudioEvent))
			{
...
{
	if(!IsValid(in_Portal) || !IsAudioAllowed())
		return;
...
bool FAkAudioDevice::IsAudioAllowed()
{
	// We don't want sound in those cases.
	return !FParse::Param(FCommandLine::Get(), TEXT("nosound")) && !FApp::IsBenchmarking() && !IsRunningDedicatedServer() && !IsRunningCommandlet();
}
...
{
	//Pinned objects are deleted during shutdown
...

	{
		...
		if (EventSetPtr)
		{
			TSet<TWeakObjectPtr<UAkAudioEvent>> EventSet = *EventSetPtr;
			for (auto AkEvent : EventSet)
			{
				...
				{
					UE_LOG(LogAkAudio, Warning, TEXT("AkEvent was destroyed before being unpinned from the GC. PlayingID : %d"), PlayingID);
				}
			}
			EventSet.Empty();
...

	{
		...
		if (MediaSetPtr)
		{
			TSet<TWeakObjectPtr<UAkExternalMediaAsset>> MediaSet = *MediaSetPtr;
			for (auto Media : MediaSet)
			{
				...
				{
					UE_LOG(LogAkAudio, Warning, TEXT("Media was destroyed before being unpinned from the GC. PlayingID : %d"), PlayingID);
				}
			}
			MediaSet.Empty();
...
FSwitchValueLifetimeEvent& FAkAudioDevice::GetOnSwitchValueLoaded(uint32 SwitchID)
{
...
{
	FSwitchValueLifetimeEvent* EventToBroadcast = OnSwitchValueLoadedMap.Find(GroupValue->ShortID);
	if (EventToBroadcast)
	{
...
FSwitchValueLifetimeEvent& FAkAudioDevice::GetOnSwitchValueUnloaded(uint32 SwitchID)
{
	return OnSwitchValueUnloadedMap.FindOrAdd(SwitchID);
}

void FAkAudioDevice::BroadcastOnSwitchValueUnloaded(UAkGroupValue* GroupValue)
{
	FSwitchValueLifetimeEvent* EventToBroadcast = OnSwitchValueUnloadedMap.Find(GroupValue->ShortID);
	if (EventToBroadcast)
	{
		EventToBroadcast->Broadcast(GroupValue);
	}
