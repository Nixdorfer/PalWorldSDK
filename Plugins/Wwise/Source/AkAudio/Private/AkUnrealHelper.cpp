...
namespace AkUnrealHelper
{
	...
	void UpdateGeneratedSoundBanksFolder(FString Platform, FString PreviousWwiseSoundBankFolder)
	{
		FString ConfigFileName = FPaths::Combine(AkUnrealHelper::GetProjectDirectory(), FString("Config"), Platform, Platform + "Game.ini");
		auto* ConfigFile = GConfig->Find(ConfigFileName, true);
		if (!ConfigFile)
		{
			return;
		}
		...
		ConfigFile->GetArray(TEXT("/Script/UnrealEd.ProjectPackagingSettings"), TEXT(".DirectoriesToAlwaysStageAsUFS"), DirectoriesArray);

		FString WwisePlatform;
		if (auto* PlatformInfo = UAkPlatformInfo::GetAkPlatformInfo(Platform))
		{
			WwisePlatform = PlatformInfo->WwisePlatform;
		}
		else
		{
			WwisePlatform = Platform;
		}

		FString PlatformFolder;
		if (const UAkSettings* AkSettings = GetDefault<UAkSettings>())
		{
			PlatformFolder = FPaths::Combine(AkSettings->WwiseSoundDataFolder.Path, WwisePlatform);
		}
		else
		{
			PlatformFolder = FPaths::Combine(UAkSettings::DefaultSoundDataFolder, WwisePlatform);
		}

		//Simulate FDirectoryPath
		const FString DirectoryPath = "(\"" + PlatformFolder + "\")";

		bool bFoundPackageDirectory = false;
		for (int i = 0; i < DirectoriesArray.Num(); i++)
		{
			if (DirectoriesArray[i] == DirectoryPath)
			{
				bFoundPackageDirectory = true;
				break;
			}
		}

		bool bDirectoriesRemoved = false;
		if (!PreviousWwiseSoundBankFolder.IsEmpty())
		{
			for (int i = 0; i < DirectoriesArray.Num(); i++)
			{
				if (DirectoriesArray[i].Contains(PreviousWwiseSoundBankFolder / WwisePlatform))
				{
					DirectoriesArray.RemoveAt(i);
					bDirectoriesRemoved = true;
				}
			}
		}

		if (!bFoundPackageDirectory)
		{
			DirectoriesArray.Add(DirectoryPath);
		}

		if (!bFoundPackageDirectory || bDirectoriesRemoved)
		{
			ConfigFile->SetArray(TEXT("/Script/UnrealEd.ProjectPackagingSettings"), TEXT("+DirectoriesToAlwaysStageAsUFS"), DirectoriesArray);
			ConfigFile->Write(ConfigFileName);
		}
	}
