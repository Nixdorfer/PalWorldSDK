...
DECLARE_EVENT_OneParam(FAkAudioDevice, FSwitchValueLifetimeEvent, UAkGroupValue*);
...
public:
	...
	*
	 ...
	 * @param in_AudioContext Context in which sound is played. Only gameplay sounds are affected by PIE pause/stop
	...
	AkPlayingID PostEvent(
		...
		const EAkAudioContext in_AudioContext = EAkAudioContext::GameplayAudio
        );
	...
	AkPlayingID PostEventAtLocation(
		...
		UWorld* in_World, EAkAudioContext in_AudioContext
	);
 ...
 */
	...
	*/
	...
	static void DelaySwitchValueLoadedBroadcast(UAkGroupValue* GroupValue) { DelayedSwitchesToBroadcast.Add(GroupValue); }
	...
	static FSwitchValueLifetimeEvent& GetOnSwitchValueLoaded(uint32 SwitchID);
	void BroadcastOnSwitchValueLoaded(UAkGroupValue* GroupValue);

	static FSwitchValueLifetimeEvent& GetOnSwitchValueUnloaded(uint32 SwitchID);
	void BroadcastOnSwitchValueUnloaded(UAkGroupValue* GroupValue);
...
private:
	...
	*/
	...
	static TMap<uint32, TSet<TWeakObjectPtr<UAkAudioEvent>>> PlayingIDToPinnedAudioEventMap;

	static FCriticalSection PlayingIDToPinnedExternalSourceMapCriticalSection;
	static TMap<uint32, TSet<TWeakObjectPtr<UAkExternalMediaAsset>>> PlayingIDToPinnedExternalSourceMap;

	static void PostEventAtLocationEndOfEventCallback(AkCallbackType in_eType, AkCallbackInfo* in_pCallbackInfo);

	static TMap<uint32, FSwitchValueLifetimeEvent> OnSwitchValueLoadedMap;
	static TMap<uint32, FSwitchValueLifetimeEvent> OnSwitchValueUnloadedMap;
};
