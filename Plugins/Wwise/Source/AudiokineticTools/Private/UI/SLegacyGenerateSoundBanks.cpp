...
FReply SLegacyGenerateSoundBanks::OnGenerateButtonClicked()
{
	...
	{
		for (auto& platformToGenerate : initParameters.Platforms)
		{
			auto* PlatformInfo = UAkPlatformInfo::GetAkPlatformInfo(*platform.Get());
			if (PlatformInfo && PlatformInfo->WwisePlatform == platformToGenerate)
			{
				AkUnrealHelper::UpdateGeneratedSoundBanksFolder(*platform.Get(), FString());
			}
		}
	}
